      ...
      >
        ...
        >
                  ...
                  @if (s1) {
                    ...
                    >
                      ...
                      @if (s1.job.id !== '00003') {
                        ...
                        @if (s1.isJobClosed) {
                          <div
                            class="mt-2 p-1.5 rounded border flex items-start gap-1.5 font-semibold text-[10px] leading-tight"
                            [class]="s1.isClosedAdmissionOnly ? 'bg-amber-50 text-amber-800 border-amber-300' : 'bg-red-100 text-red-800 border-red-200'"
                          >
                            <span>⚠</span>
                            <div>
                    ...
                    >
                      ...
                      @if (s2.job.id !== '00003') {
                        ...
                        @if (s2.isJobClosed) {
                          <div
                            class="mt-2 p-1.5 rounded border flex items-start gap-1.5 font-semibold text-[10px] leading-tight"
                            [class]="s2.isClosedAdmissionOnly ? 'bg-amber-50 text-amber-800 border-amber-300' : 'bg-red-100 text-red-800 border-red-200'"
                          >
                            <span>⚠</span>
                            <div>
                    ...
                    >
                      ...
                      @if (s3.job.id !== '00003') {
                        ...
                        @if (s3.isJobClosed) {
                          <div
                            class="mt-2 p-1.5 rounded border flex items-start gap-1.5 font-semibold text-[10px] leading-tight"
                            [class]="s3.isClosedAdmissionOnly ? 'bg-amber-50 text-amber-800 border-amber-300' : 'bg-red-100 text-red-800 border-red-200'"
                          >
                            <span>⚠</span>
                            <div>
          ...
          @if (!isPforApplicant()) {
            ...
            <!-- Volet PFOR lorsque Postulant PFOR est activé -->
            <div class="p-5 bg-white border border-indigo-200 rounded-xl shadow-sm flex flex-col gap-4">
                ...
                <div class="pt-3 border-t border-slate-100 flex flex-col gap-2">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    >
                      <input
                        type="checkbox"
                        class="peer h-4 w-4 appearance-none rounded border border-slate-300 bg-white checked:bg-indigo-600 checked:border-indigo-600 focus:outline-none transition-all cursor-pointer"
                        [checked]="cmrArts()"
  ...
  evaluateJobAdmissibility(jobId: string) {
    ...
    if (jobId === "00003") {
      return {
        ...
        durationYears: 3,
        isJobClosed: false,
        isClosedTraitement: false,
        isClosedAdmissionOnly: false,
      };
    }
    ...
    if (this.isPforApplicant()) {
      ...
      const hasPforTraitement = this.jobService.hasPforProgram(jobId, "traitement");
      const isPforClosedTraitement = this.jobService.isPforJobClosed(jobId, "traitement");
      const isClosedTraitementRaw = !hasPforTraitement || isPforClosedTraitement;

      const hasPforAdmission = this.jobService.hasPforProgram(jobId, "admission");
    ...
    for (const id of realDossierIds) {
      ...
      if (s) {
        if (
          ...
          if (s.isClosedAdmissionOnly) {
            closedAdmissionOnlyJobs.push(id);
          } else {
        ...
        for (const id of realDossierIds) {
          ...
          if (s) {
            ...
            if (s.isJobClosed) {
              if (s.isClosedAdmissionOnly) {
                reasonsFrList.push(
                  "Nous avons déjà un nombre suffisant de candidatures pour combler toutes les positions restantes.",
                );
              } else if (this.isPforApplicant()) {
                reasonsFrList.push(
        ...
        for (const id of realDossierIds) {
          ...
          if (s) {
            ...
            if (s.isJobClosed) {
              if (s.isClosedAdmissionOnly) {
                reasonsEnList.push(
                  "We already have a sufficient number of applications to fill all remaining positions.",
                );
              } else if (this.isPforApplicant()) {
                reasonsEnList.push(
